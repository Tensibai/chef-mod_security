...
[:mod_security][:dir]
  ...
  default[:mod_security][:mlogc][:bin]                 = "#{node[:mod_security][:dir]}/bin/mlogc.bat}"

else
  default[:mod_security][:audit_dir]                    = '/var/log'
  default[:mod_security][:audit_log]                    = "#{node[:mod_security][:audit_dir]}/modsec_audit.log"
  default[:mod_security][:audit_context]                = 'httpd_log_t'
  default[:mod_security][:mlogc][:bin]                 = '/usr/bin/mlogc'

end
# concurrent audit logging used in templates only if audit_log_type is 'Concurrent' (case sensitive))
# Mlogc configuration, used only if console is not nil
# TODO: Allow all mlog.conf parameters.
default[:mod_security]
default[:mod_security][:mlogc][:consoleuri]		= nil # ex: https://127.0.0.1:8888
default[:mod_security][:mlogc][:sensor]                 = { "name" => "sensor", "pass" => "sensor" }
default[:mod_security][:mlogc][:storagedir]             = "audit" # relative to audit_dir attribute
